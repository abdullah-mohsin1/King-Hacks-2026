...
# OpenAI (recommended - works reliably)
OPENAI_API_KEY=
...
### 3. Set Up Python Environment (Required for LLM)

The project uses Python with LangChain for intelligent note generation. **OpenAI is recommended** for the most reliable experience:
...
- May have model availability issues
...
Test the complete pipeline (speech-to-text → LLM → notes):

```bash
# Test the LLM pipeline with sample data
python scripts/test_pipeline.py
```

This will show:
1. Sample transcript (simulated speech-to-text)
2. Short notes generated by LLM
3. Detailed notes generated by LLM
